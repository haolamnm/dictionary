...
std::vector<std::shared_ptr<Word>> Tree::suggest(const std::string &prefix) const {
    ...
    suggest_core(node, prefix, suggestions);
    return suggestions;
}

std::vector<std::shared_ptr<Word>> Tree::match(const std::string &pattern) const {
    std::vector<std::shared_ptr<Word>> matches;
    Node *node = root.get();
    match_core(node, 0, "", pattern, matches);
    return matches;
}
...
void Tree::suggest_core(const Node *node, std::string prefix,
    ...
    for (const auto &[c, child] : node->get_children()) {
        suggest_core(child.get(), prefix + c, suggestions);
    }
}

void Tree::match_core(const Node *node, size_t pattern_index, const std::string &current,
                      const std::string &pattern,
                      std::vector<std::shared_ptr<Word>> &matches) const {
    // End of finding path
    ...
    if (pattern_char == '*') {
        ...
        match_core(node, pattern_index + 1, current, pattern, matches);

        // Case 2. Match one or more characters (recurse into each child and stay on '*')
        for (const auto &[c, child] : node->get_children()) {
            match_core(child.get(), pattern_index, current + c, pattern, matches);
        }
    }
    // '+': Must match at least one or more characters
        ...
        for (const auto &[c, child] : node->get_children()) {
            ...
            match_core(child.get(), pattern_index + 1, current + c, pattern, matches);
        }
    }
